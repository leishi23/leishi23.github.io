...
### Body frame Newton-Euler Equation: $\left[\begin{matrix}mI & 0 \\ 0 & \mathcal{I}\end{matrix}\right] \left[\begin{matrix}\dot{v^b} \\ \dot{\omega^b}\end{matrix}\right] + \left[\begin{matrix}\omega^b \times mv^b \\ \omega^b \times \mathcal{I}\omega^b\end{matrix}\right]=\left[\begin{matrix}F^b \\ \tau^b\end{matrix}\right]$
  ...
  - $T$: Kinetic Energy
    - If T of a rigid body, then $T=\frac{1}{2}m\dot{q}^T\dot{q}+\frac{1}{2}\omega^T I \omega$
      ...
      - Derivation:
        ...
        - with generalized inertia matrix $M=\left[\begin{matrix}mI_3 & 0 \\ 0 & \mathcal{I}\end{matrix}\right]$ where $I_3$ is a 3x3 identity matrix and $\mathcal{I}$ is the inertia matrix of the rigid body.
        - then $T=\frac{1}{2}{\vec{v^b}}^TM{\vec{v^b}}$ where $\vec{v^b}=\left[\begin{matrix}R\dot{q} \\ \omega\end{matrix}\right]$, the generalized velocity w.r.t. the body frame.
